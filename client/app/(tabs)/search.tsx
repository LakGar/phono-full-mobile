...
import { SFSymbols6_0 } from "@/components/ui/SFSymbols6_0";
...
const SearchScreen = () => {
  ...
  const paddingTopAnim = useRef(new Animated.Value(140)).current;
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success && data.albums) {
          ...
          console.log("Setting search results:", processedAlbums);
          setSearchResults(processedAlbums);
        } else {
          setError("No albums found for this genre");
        }
      } else {
        const errorData = await response.json();
        setError(errorData.message || "Failed to fetch genre albums");
      }
    } catch (error) {
      console.error("Error fetching genre albums:", error);
      setError("Failed to fetch genre albums. Please try again.");
    } finally {
      setLoading(false);
    }
            ...
            >
              {categories.map((category, index) => (
                ...
                >
                  ...
                  >
                    <IconSymbol
                      name={category.icon as SFSymbols6_0}
                      size={24}
                      color="#FFFFFF"
                    />
                  </View>
